...
public class NotificationServiceClient {
	...
	public NotificationServiceClient(@LoadBalanced RestClient.Builder loadBalancedRestClientBuilder,
			RestClient.Builder restClientBuilder,
			@Value("${project.client.notification-service-url:http://localhost:8087}") String notificationServiceUrl) {
	...
	public void send(NotificationCommand request, String authorizationHeader) {
		try {
	...
	private void send(RestClient restClient, NotificationCommand request, String authorizationHeader) {
		try {
			restClient.post()
			...
			LOGGER.warn("Unable to dispatch project notification for recipient {}: {}",
